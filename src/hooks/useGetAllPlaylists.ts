...
const BATCH_SIZE = 3; // Number of playlists to process simultaneously
  ...
  useEffect(() => {
    ...
    const fetchAllPlaylists = async () => {
      try {
        ...
        // Process playlists in batches instead of all at once
        const playlistsWithTracks = await processBatch(userPlaylists, user);
